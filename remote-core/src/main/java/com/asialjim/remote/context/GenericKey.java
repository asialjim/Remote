...
/**
 * <pre>
 * Remote 上下文通用Hash Key, 用于在 {@link  RemoteGenericMap} 中，的Key, 并且可用于不同的Key，其Value具有不同的类型
 *
 * "Remote Context Generic Hash Key" is used as a key in the {@link RemoteGenericMap} and can be employed for different keys where their corresponding values have different types.
 * </pre>
 *
 * @param <Value> {@link RemoteGenericMap} Value Java Type
